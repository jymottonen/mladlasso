#' Stability selection
#'
#' \code{stability} is used to calculate the stability selection frequencies.
#'
...
#' @param efp the expected number of false positives we want to control. The default value is 1.
#' @details 
#' Here are the details of the function...
#' @return A list containing the following components:
#' \describe{
#' \item{freq}{a q-vector,recording the stability frequencies of each marker over B replicates.}
#' \item{markersig}{the indices of markers which are judged to be significant by stability selection.}
#' }
#' @references 
...
#' @import SpatialNP
#' @export
stability<-function(Y, X, initialB=NULL, lambda1=0, lambda2=0, 
                    method="BFGS",gradient=FALSE,functional=0,
                    reltol=1e-8,trace=0,N=100,efp=1)
{
  ...
  for(i in 1:N)
  {
    ...
    if(functional==0|lambda2==0)
    {
      ...
      freq[which(norms>1e-4)-1] <- freq[which(norms>1e-4)-1]+1/N  #calculate the selection frequency
      qn[i]<-sum(norms>1e-4)-1
    }
    else 
    {
  ...
  markersig <- which(freq > 1/2 + mean(qn)^2/(n*efp*2)) #detect significant markers
  
  fit<-list(freq=freq, markersig = markersig)
  class(fit) <- "stability"
  return(fit)
}
